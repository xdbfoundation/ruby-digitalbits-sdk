RSpec.describe DigitalBits::Frontier::Client do
  ...
  describe "headers" do
    ...
    it "has 'X-Client-Name'" do
      expect(headers["X-Client-Name"]).to eq "ruby-digitalbits-sdk"
    end

    it "has 'X-Client-Version'" do
      expect(headers["X-Client-Version"]).to eq DigitalBits::Frontier::VERSION
    end
  end

  describe "#default_testnet" do
    it "instantiates a client pointing to frontier testnet" do
      client = described_class.default_testnet
      expect(client.frontier._url).to eq(described_class::FRONTIER_TESTNET_URL)
    end
  end

  describe "#default" do
    it "instantiates a client pointing to frontier mainnet" do
      client = described_class.default
      expect(client.frontier._url).to eq(described_class::FRONTIER_MAINNET_URL)
    end
  end

  describe "#localhost" do
    it "instantiates a client pointing to localhost frontier" do
      client = described_class.localhost
      expect(client.frontier._url).to eq(described_class::FRONTIER_LOCALHOST_URL)
    end
  end

  describe "#initialize" do
    let(:custom_frontier_url) { "https://frontier.domain.com" }
    it "instantiates a client accepting custom options" do
      client = described_class.new(frontier: custom_frontier_url)
      expect(client.frontier._url).to eq(custom_frontier_url)
    end
  end
  ...
  describe "#friendbot" do
    ...
    it("requests for XDB from a friendbot", {
  ...
  describe "#send_payment" do
    ...
    context "using a payment channel" do
      ...
      it("sends a payment account through a channel account", {
        ...
        tx = client.frontier.transaction(hash: tx_hash)
  ...
  describe "#transactions" do
    ...
    context "account transactions" do
      ...
      it "returns a list of transaction for an account", vcr: {record: :once, match_requests_on: [:method]} do
        ...
        expect(response).to be_a(DigitalBits::Frontier::TransactionPage)
      end
  ...
  describe "#submit_transaction" do
    ...
    it("raises an error for missing memo", {
      ...
      expect {
        ...
        an_instance_of(
          DigitalBits::Frontier::AccountRequiresMemoError
        ).and(
          having_attributes(
    ...
    context "when tx is fee bump" do
      ...
      it "raises an error for missing memo", vcr: {record: :once, match_requests_on: [:method]} do
        ...
        expect { client.check_memo_required(envelope) }.to raise_error(
          an_instance_of(DigitalBits::Frontier::AccountRequiresMemoError).and(
            having_attributes(
              message: "account requires memo",
              account_id: memo_required_kp.muxed_account,
              operation_index: 0
